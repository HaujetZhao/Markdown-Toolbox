...
class Thread_ClearAttatchment(QThread):
    ...
    def run(self):
        ...
        for 输入文件夹 in 输入文件列表:
            ...
            当前文件夹下的附件路径列表 = []
            当前文件夹下的存储相对路径附件的文件夹列表 = []
            if not os.path.exists(输入文件夹):
                continue
            输入文件夹下的文件目录 = os.listdir(输入文件夹)
            md文档目录 = []
            print(f'当前搜索文件夹的md文档有：')
            for 文件路径 in 输入文件夹下的文件目录:
                if re.match('.+\.md$', 文件路径):
                    md文档目录.append(文件路径)
                    print(f'      {文件路径}')
            if len(md文档目录) < 1:
                ...
                for 附件路径 in 搜索到的附件路径列表:
                    ...
                    if os.path.exists(附件路径加上文档路径转换的绝对路径):
                        if 附件路径加上文档路径转换的绝对路径 not in 当前文件夹下的附件路径列表:
                            ...
                            print(f'   {附件路径}')
                        if os.path.basename(附件路径加上文档路径转换的绝对路径) not in 当前文件夹下的存储相对路径附件的文件夹列表:
                            当前文件夹下的存储相对路径附件的文件夹列表.append(os.path.basename(附件路径加上文档路径转换的绝对路径))
            已找到的附件文件夹中的文件列表 = []
            for 附件文件夹 in 输入文件夹下的文件目录:
            ...
            print('现在开始移动无用的附件')
            for 无用附件 in 无用的附件列表:
                目标文件夹 = os.path.dirname(os.path.dirname(无用附件)) + '/' + '无用附件'
                if not 检查路径(目标文件夹):
