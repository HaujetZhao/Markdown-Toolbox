...
class Thread_TransportFile(QThread):
    ...
    def run(self):
        ...
        常量.主窗口.setWindowTitle(常量.主窗口.窗口标题 + '（执行中……）')
        复制和移动文档线程常量.判断文件是否相同的方式 = 0 # 0 是覆盖, 1 是全部覆盖 2 是跳过, 3 是全部跳过，
        for 输入文件 in self.输入文件列表:
            ...
            print('开始转移文档')
            try:
                转移文件(输入文件, md文件的复制输出路径, self.转移md文档时是否为移动, self.提醒是否确认要覆盖的信号, 复制和移动文档线程常量)
            except:
                print(f'无法将 {输入文件} 移动到指定位置 {md文件的复制输出路径}')
                continue
        复制和移动文档线程常量.判断文件是否相同的方式 = 0


        常量.状态栏.showMessage('任务完成')
        常量.主窗口.setWindowTitle(常量.主窗口.窗口标题 + '（完成）')
        for 组件 in self.执行期间需要禁用的组件:
            组件.setEnabled(True)
        print('\n\n清理完成\n\n')
...
def 将文档索引的附件复制(文档, 附件列表, 目标文件夹):
    ...
    for 附件路径 in 附件列表:
        if os.path.exists(文档所在文件夹 + '/' + os.path.dirname(附件路径)): # 如果这个图片路径是个相对路径
            if not self.检查文件夹路径(目标文件夹 + '/' + os.path.dirname(附件路径)): # 创建目标相对路径
